...
class UserTask(TaskSet):
    # 用户名密码登录
    def on_start(self):
        for index in range(self.locust.tokenLength):
            mobile = random.choice(['139', '188', '185', '136', '158', '151']) + "".join(
                random.choice("0123456789") for i in range(8))
            with self.client.post("/token/_verifyMobile",
                                  headers=self.locust.header,
                                  data={"mobile": mobile, "smscode": "123"},
                                  catch_response=True) as response:
                print random.randint(0, self.locust.tokenLength - 1)
                # print response.json()
                # print response.status_code
                if response.status_code == 200:
                    response.success()
                    content = json.loads(response.content)
                    self.locust.accessTokens[index] = content["accessToken"]
                else:
    ...
    #     with self.client.delete("/token", headers={"Authorization": "Bearer %s" % self.locust.accessTokens[
    #                                       random.randint(0, self.locust.tokenLength - 1)]},
    #                             catch_response=True, ) as response:
    #         if response.status_code == 200:
    #             response.success()
    #         else:
    ...
    #         if response.status_code == 200:
    #             content = json.loads(response.content)
    #             self.locust.accessTokens[
    #                                       random.randint(0, self.locust.tokenLength - 1)] = content["accessToken"]
    #             response.success()
    #         else:
    #             print response.content
    #             response.failure(response.content)
    ...
    def user_profile(self):
        with self.client.get("/user/profile",
                             headers={
                                 "Authorization": "Bearer %s" % self.locust.accessTokens[
                                     random.randint(0, self.locust.tokenLength - 1)]},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
    ...
    def other_user_profile(self):
        with self.client.get("/users/%s/profile" % self.locust.userId,
                             headers={"Authorization": "Bearer %s" % self.locust.accessTokens[
                                 random.randint(0, self.locust.tokenLength - 1)]},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
    ...
    def post_user_tags(self):
        with self.client.post("/user/tags",
                              headers={"Authorization": "Bearer %s" % self.locust.accessTokens[
                                  random.randint(0, self.locust.tokenLength - 1)], "X-Client": "User"},
                              json={
                                  "codes": ["91", "92"]
                              },
                              catch_response=True) as response:
