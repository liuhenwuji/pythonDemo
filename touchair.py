...
class UserTask(TaskSet):
    ...
    #     with self.client.delete("/token", headers={"Authorization": "Bearer %s" % self.accessToken},
    #                             catch_response=True, ) as response:
    #         if response.status_code == 200:
    #             response.success()
    #         else:
    ...
    def get_mobile(self):
        try:
            return self.locust.mobile_queue.get()
        except queue.Empty:
            print('account data run out, test ended.')
            exit(0)
    ...
    #                           headers={"Authorization": "Bearer %s" % self.accessToken,'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:4.0) Gecko/20100101 Firefox/24.0'},
    #                           json={
    #                               "openId": "xinghai"
    #                           },
    #                           catch_response=True) as response:
    ...
    #                           headers={"Authorization": "Bearer %s" % self.accessToken,'User-Agent': 'Mozilla/5.5 (Windows NT 6.0; WOW64; rv:.0) Gecko/20100101 Firefox/24.0'},
    #                           json={
    ...
    #                           headers={"Authorization": "Bearer %s" % self.accessToken,'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/24.0'},
    #                           json={
    ...
    #                             headers={"Authorization": "Bearer %s" % self.accessToken,'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:9.0) Gecko/20100101 Firefox/24.0'},
    #                             catch_response=True) as response:
    #         if response.status_code == 200:
    #             response.success()
    #         else:
    ...
    #                             headers={"Authorization": "Bearer %s" % self.accessToken,'User-Agent': 'Mozilla/5.0 (Windows NT 6.0; WOW64; rv:10.0) Gecko/20100101 Firefox/24.0'},
    #                             catch_response=True) as response:
    #         if response.status_code == 200:
    #             response.success()
    #         else:
    ...
    def post_user_tags(self):
        with self.client.post("/user/tags",
                              headers={"Authorization": "Bearer %s" % self.accessToken, "X-Client": "User"},
                              json={
                                  "codes": ["91", "92"]
                              },
                              catch_response=True) as response:
    ...
    #                          headers={"Authorization": "Bearer %s" % self.accessToken,
    #                                   'User-Agent': 'Mozilla/4.7 (Windows NT 6.0; WOW64; rv:25.0) Gecko/20100101 Firefox/24.0'},
    #                          catch_response=True) as response:
    #         if response.status_code == 200:
    #             response.success()
    #         elif response.status_code != 0:
    ...
    def users_actors(self):
        with self.client.get("/users/%s/actors" % self.locust.other_userId,
                             headers={"Authorization": "Bearer %s" % self.accessToken,
                                      'User-Agent': 'Mozilla/4.8 (Windows NT 6.0; WOW64; rv:26.0) Gecko/20100101 Firefox/24.0'},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            elif response.status_code != 0:
    ...
    def search(self):
        with self.client.get("/poi/actor/%s" % self.locust.actorId,
                             headers={"Authorization": "Bearer %s" % self.accessToken,
                                      'User-Agent': 'Mozilla/3.3 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/34.0'},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            elif response.status_code != 0:
    ...
    def user_trace_report(self):
        with self.client.get("/usertracereport",
                             headers={"Authorization": "Bearer %s" % self.accessToken,
                                      'User-Agent': 'Mozilla/3.5 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/37.0'},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            elif response.status_code != 0:
    ...
    def user_trace_report_image(self):
        with self.client.get("/usertracereportimage",
                             headers={"Authorization": "Bearer %s" % self.accessToken,
                                      'User-Agent': 'Mozilla/3.6 (Windows NT 6.0; WOW64; rv:24.0) Gecko/20100101 Firefox/38.0'},
                             catch_response=True) as response:
            if response.status_code == 200:
                response.success()
            elif response.status_code != 0:
...
class User(HttpLocust):
    ...
    for index in range(50000):
        ...
        mobile_queue.put_nowait(mobile)
